...
- Required Debian/Ubuntu packages installed in Dockerfile:
  ...
  - `ttf-mscorefonts-installer` (installs Times New Roman from Microsoft; requires `contrib` repo and internet access during Docker build; EULA pre-accepted non-interactively via `debconf-set-selections`)
